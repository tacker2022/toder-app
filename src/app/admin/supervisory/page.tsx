...
export default async function AdminSupervisoryPage({
    searchParams,
}: {
    searchParams?: {
        query?: string;
    ...
    const members = await getMembers(query, company, "supervisory");

    // Get unique companies for filter
    const allMembers = await getMembers(undefined, undefined, "supervisory");
    const companies = Array.from(new Set(allMembers.map(m => m.company).filter(Boolean))) as string[];

    return (
        <div>
            <div className="flex justify-between items-center mb-8">
                <h1 className="text-3xl font-bold">Denetim Kurulu</h1>
                <Link
                    href="/admin/supervisory/new"
                ...
                {members.map((member: any) => (
                    ...
                    >
                        ...
                        <div className="flex items-center gap-2">
                            <Link
                                href={`/admin/supervisory/${member.id}`}
