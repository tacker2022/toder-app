...
import { updateLegislation } from "@/actions/legislations";
import { useRouter } from "next/navigation";
import Link from "next/link";
import { ArrowLeft, Save, Upload, FileText } from "lucide-react";
import { useState } from "react";

export default function EditLegislationForm({ legislation }: { legislation: any }) {
    const router = useRouter();
    const [loading, setLoading] = useState(false);

    async function handleSubmit(formData: FormData) {
        setLoading(true);
        const result = await updateLegislation(legislation.id, formData);
        setLoading(false);
    ...
    return (
        <>
            <div className="mb-8">
                ...
                <h1 className="text-3xl font-bold text-white">Belgeyi Düzenle</h1>
            </div>
            ...
            <form action={handleSubmit} className="space-y-6 bg-[#0a0a0a] border border-white/10 p-8 rounded-xl">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select
                            name="category"
                            defaultValue={legislation.category}
                            required
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input
                            type="date"
                            name="published_date"
                            defaultValue={legislation.published_date}
                            className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-[#D4AF37] transition-colors"
                        />
                    </div>
                    ...
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <div className="w-full bg-white/5 border border-dashed border-white/20 rounded-lg px-4 py-3 text-white/60 flex items-center gap-2">
                                <Upload size={20} />
                                <span>Yeni PDF seçmek için tıklayın (Opsiyonel)</span>
                            </div>
                        </div>
                        {legislation.pdf_url && (
                            <div className="mt-2 flex items-center gap-2 text-sm text-[#D4AF37]">
                                <FileText size={14} />
                                <a href={legislation.pdf_url} target="_blank" rel="noopener noreferrer" className="hover:underline">
